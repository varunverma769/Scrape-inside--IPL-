...
function getMatchInfo(idx,err,res,html) {
    if(!err) {
        ...
        for(let i = 0; i < tables.length; i++) {
            ...
            for(let j = 0; j < rows.length; j+=2) { // one row is empty after each row 
                let columns = $(rows[j]).find("td"); // each batsmen data
                let playerInfo = {};
                for(let k = 0; k < columns.length; k++) {
                    if(k != 1 && k != 4 ) {   // because first and fourth 'td' is not of use.
